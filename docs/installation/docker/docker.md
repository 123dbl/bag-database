...
| `AMAP_API_KEY` | An Amap/Gaode Web Service API key used to resolve GPS coordinates into place names with the reverse geocoding API. This is only needed for automatic location names; maps and track display do not require it. See the [Amap reverse geocoding documentation](https://lbs.amap.com/api/webservice/guide/api/georegeo/). | |
...
| `SATELLITE_TILE_MAP_URL` | The satellite tile URL available from the map window base-layer selector. The default value uses the Amap/Gaode satellite GCJ02 tile map. | https://webst02.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z} |
